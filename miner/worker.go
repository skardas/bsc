...
func newWorker(config *Config, chainConfig *params.ChainConfig, engine consensus.Engine, eth Backend, mux *event.TypeMux, isLocalBlock func(*types.Block) bool, init bool, flashbots *flashbotsData) *worker {
	...
	newWorker.txsSub = eth.TxPool().SubscribeNewTxsEvent(newWorker.txsCh)
	// Subscribe events for blockchain
	newWorker.chainHeadSub = eth.BlockChain().SubscribeChainHeadEvent(newWorker.chainHeadCh)
	newWorker.chainSideSub = eth.BlockChain().SubscribeChainSideEvent(newWorker.chainSideCh)

	// Sanitize recommit interval if the user-specified one is too short.
	recommit := newWorker.config.Recommit
	if recommit < minRecommitInterval {
		log.Warn("Sanitizing miner recommit interval", "provided", recommit, "updated", minRecommitInterval)
		recommit = minRecommitInterval
	}

	go newWorker.mainLoop()
	go newWorker.newWorkLoop(recommit)
	if !flashbots.isFlashbots {
		// only mine if not flashbots
		go newWorker.resultLoop()
		go newWorker.taskLoop()
	}
	// Submit first work to initialize pending state.
	go func(init bool, newWorker *worker) {
		time.Sleep(10 * time.Second)
		if init {
			newWorker.startCh <- struct{}{}
		}
	}(init, newWorker)
